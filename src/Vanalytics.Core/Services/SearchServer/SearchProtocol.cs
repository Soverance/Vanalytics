...
{
    private static readonly byte[] _keySeed =
    {
    ...
    public static ReadOnlySpan<byte> KeySeed => _keySeed;
    ...
    public const int OffStack  = 0x15;
    public const int MinPacket = 28;

    public static byte[] Md5(ReadOnlySpan<byte> input)
    {
        Span<byte> dst = stackalloc byte[16];
        MD5.HashData(input, dst);
        return dst.ToArray();
    }
}
...
public sealed class Blowfish
{
    ...
    // P-array and S-boxes sourced verbatim from LandSandBoat server/src/common/blowfish.cpp
    // (standard Blowfish initialization constants — hex digits of pi — under GPL-3.0)
    ...
    {
        if (key.Length == 0) throw new ArgumentException("Blowfish key must be non-empty", nameof(key));

        Array.Copy(PInit, _p, 18);
        for (int i = 0; i < 4; i++) Array.Copy(SInit[i], _s[i], 256);
...
#pragma warning restore CS0675
        ...
        uint xl = 0, xr = 0;
        for (int i = 0; i < 18; i += 2) { EncipherBlock(ref xl, ref xr); _p[i] = xl; _p[i + 1] = xr; }
        for (int i = 0; i < 4; i++)
...
public readonly record struct SearchKeyContext(uint Nonce, uint ResponseSeed);
