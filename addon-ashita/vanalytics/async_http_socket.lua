-- addon-ashita/vanalytics/async_http_socket.lua
-- Non-blocking HTTP/HTTPS client for the Vanalytics addon (LuaSocket + LuaSec).
-- This is the ORIGINAL Windower implementation, kept verbatim. It is selected
-- automatically by async_http.lua ONLY when a real luasocket ('socket') and
-- luasec ('ssl') are require-able in the Ashita LuaJIT environment. When they
-- are not (the common case on Ashita v4), async_http.lua falls back to the
-- WinHTTP transport in async_http_winhttp.lua instead. See PORTING.md.
--
